...
const ImageSnake = ({ images }: { images: ImageGalleryItem[] }) => {
  ...
  const calculateNextPosition = (
    ...
    const angle =
      pathTime * 0.1 +
      Math.sin(pathTime * 0.07) * 2 * angleDirectionRef.current +
      randomOffset;
    ...
    if (hitLeft || hitRight || hitTop || hitBottom) {
      nextX = Math.max(
        MARGIN,
        Math.min(containerWidth - SEGMENT_SIZE - MARGIN, nextX)
      );
      nextY = Math.max(
        MARGIN,
        Math.min(containerHeight - SEGMENT_SIZE - MARGIN, nextY)
      );
      angleDirectionRef.current = Math.random() > 0.5 ? 1 : -1;
      pathTimeRef.current += Math.PI / 2 + Math.random();
    }
  ...
  const addNewSegment = useCallback(() => {
    ...
    const findValidPosition = (prev: Segment[]): { x: number; y: number } => {
      ...
      const escapeX = Math.max(
        MARGIN,
        Math.min(
          containerWidth - SEGMENT_SIZE,
          lastPos.x + Math.cos(escapeAngle) * escapeDistance
        )
      );
      const escapeY = Math.max(
        MARGIN,
  ...
  if (!mounted)
    return (
      <div
        ref={containerRef}
        className="relative w-full h-[calc(100vh-268px)] overflow-hidden"
      />
    );
          ...
          {segments.map((seg, i) => (
            ...
            >
              {seg.image ? (
                ...
                <div>
                  <p>Ikke noe bilde, eller feil med bilde</p>
                </div>
              )}
            </motion.div>
          ))}
